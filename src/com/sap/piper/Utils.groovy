...
def unstash(name, msg = "Unstash failed:") {
    ...
    try {
        ...
        if (e.getMessage().contains("JNLP4-connect")) {
            ...
            // in case of an AccessDeniedException, check if it is causes by pack file from .git
            // if an .git folder already exists on the workspace, it contains read-only files which
